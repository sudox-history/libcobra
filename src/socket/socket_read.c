...
void cobra__socket_read_callback(uv_stream_t *tcp_handle,
    ...
    while (true) {
        if (!sock->read_frame_body_length) {
            ...
            if (cobra_buffer_length(&sock->read_buffer) < COBRA_SOCKET_FRAME_HEADER_LENGTH)
                break;

            // Reading header
            sock->read_frame_body_length =
                    cobra_buffer_read_uint(&sock->read_buffer, COBRA_SOCKET_FRAME_HEADER_LENGTH);
        ...
        if (sock->read_frame_body_length > cobra_buffer_length(&sock->read_buffer))
            break;
        ...
        if (alloc_callback != NULL && read_callback != NULL) {
            ...
            cobra_buffer_read(&sock->read_buffer, packet_body, sock->read_frame_body_length);
            read_callback(sock, packet_body, sock->read_frame_body_length);
        }
        else
            cobra_buffer_read_void(&sock->read_buffer, sock->read_frame_body_length);
