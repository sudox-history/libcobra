...
void cobra__socket_write_async_send_callback(cobra_async_t *async,
    ...
    cobra_buffer_init(&uv_write_context->frame,
                      length + COBRA_SOCKET_FRAME_HEADER_LENGTH);
    uv_req_set_data((uv_req_t *)&uv_write_context->request, uv_write_context);

    cobra_buffer_write_uint(&uv_write_context->frame, length,
    ...
    uv_write(&uv_write_context->request, (uv_stream_t *)&sock->tcp_handle,
             &write_buffer, 1, cobra__socket_write_callback);
}
...
void cobra__socket_write_callback(uv_write_t *write_request, int error) {
    cobra__socket_uv_write_ctx_t *uv_write_ctx =
        uv_req_get_data((uv_req_t *)write_request);
