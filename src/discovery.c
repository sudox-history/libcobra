...
void cobra__discovery_close(cobra_discovery_t *discovery, int error) {
    ...
    uv_close((uv_handle_t *)&discovery->udp_handle, cobra__discovery_on_close);
}
...
void cobra__discovery_on_alloc(uv_handle_t *handle,
                               size_t _,
                               uv_buf_t *read_buffer) {
    cobra_discovery_t *discovery = handle->data;

    read_buffer->base =
        (char *)cobra_buffer_write_pointer(&discovery->read_buffer);
...
void cobra__discovery_on_listener_data(uv_udp_t *handle,
    ...
    uv_udp_send(send_req, &discovery->udp_handle, &send_buffer, 1, addr,
...
int cobra_discovery_listen(cobra_discovery_t *discovery) {
    ...
    if (uv_udp_bind(&discovery->udp_handle, (const struct sockaddr *)&bind_addr,
                    0))
    ...
    uv_udp_recv_start(&discovery->udp_handle, cobra__discovery_on_alloc,
...
void cobra__discovery_on_scanner_time(uv_timer_t *handle) {
    ...
    uv_udp_send(send_req, &discovery->udp_handle, &send_buffer, 1,
                (const struct sockaddr *)&addr, cobra__discovery_on_send);
}
...
int cobra_discovery_scan(cobra_discovery_t *discovery) {
    ...
    uv_timer_start(&discovery->timer_handle, cobra__discovery_on_scanner_time,
                   0, COBRA_DISCOVERY_TIMEOUT);
